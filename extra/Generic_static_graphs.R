...
if (is.null(country)) { country <- "Germany" }

dataset_details <- get_available_datasets("regional") %>% filter(grepl(country, origin))

level_1_data  <- get_regional_data(country=country,
                                   totals=FALSE,
                                   level = 1,
                                   localise = FALSE)

if (!is.na(dataset_details$level_2_region)) {
  level_2_data <- get_regional_data(country=country,
                                    totals=FALSE,
                                    level = 2,
                                    localise = FALSE)
}
...
if (!is.na(dataset_details$level_2_region)) {
  region_summaries <-
    ...
    summarise(min_i=min(cases_new, na.rm=TRUE),
...
if (!is.na(dataset_details$level_2_region)) {
  ...
  narrowed_regional_incidence %>%
    ggplot(aes(x=date,y=-as.numeric(factor(level_2_region))*intercity_gap,
               height=cases_new, group=level_2_region)) + #y=as.numeric(level_2_region)*250
  ...
  ggsave(paste0("extra/", country, "-ridgeline-top-level-2-regions.png"))
}
...
level_1_data %>%
  filter(level_1_region!="Unknown") %>%
  ggplot(aes(x=date,y=-as.numeric(factor(level_1_region))*intercity_gap,
             height=cases_new, group=level_1_region)) + #y=as.numeric(level_2_region)*250
  ...
  theme(axis.text.y = element_text(size=8),
        axis.title.y.left = element_blank())
ggsave(paste0("extra/", country, "-ridgeline-all-level-1-regions.png"))
...
if (!is.na(dataset_details$level_2_region)) {
  ...
  level_2_counts %>%
    ...
    labs(x="Date",
         ...
         fill="Case count",
         title=paste0(tools::toTitleCase(dataset_details$level_2_region),
                      " case counts in ", country),
         subtitle="7 day average counts of new cases",
         caption=caption_text) +
    scale_fill_brewer(palette="Blues",direction=1)

  ggsave(paste0("extra/", country, "-waterfall-case-counts-level-2.png"))
}
...
if (!is.na(unique(level_1_data$tested_new))) {
  ...
  ggsave(paste0("extra/", country, "-waterfall-positivity.png"))
}
...
if (!is.na(unique(national_data$tested_new))) {
  ...
  ggsave(paste0("extra/", country, "-acceleration-national.png"))
}
